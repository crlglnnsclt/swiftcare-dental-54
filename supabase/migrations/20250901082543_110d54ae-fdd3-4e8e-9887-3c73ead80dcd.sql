-- Update the queue position trigger to handle emergency priority better
DROP TRIGGER IF EXISTS trigger_calculate_queue_position ON appointments;
DROP FUNCTION IF EXISTS calculate_enhanced_queue_position;
...
-- Create function to handle grace period expiration
CREATE OR REPLACE FUNCTION public.handle_grace_period_expiration()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Mark appointments as no-show if they haven't checked in after grace period
  UPDATE appointments 
  SET 
    status = 'no-show',
    is_no_show = true,
...
-- Update existing appointments to have proper grace periods
UPDATE appointments 
SET grace_period_end = appointment_date + INTERVAL '15 minutes'
...
ON appointments (appointment_date, is_checked_in, status, branch_id, queue_position);

-- Add index for grace period queries
CREATE INDEX IF NOT EXISTS idx_appointments_grace_period 
ON appointments (grace_period_end, is_checked_in, status) 
