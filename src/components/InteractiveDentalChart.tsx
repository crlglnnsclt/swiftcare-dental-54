...
export default function InteractiveDentalChart() {
  ...
  function mutate(mutator) { setState(prev => { const snapshot = JSON.parse(JSON.stringify(prev)); const next = mutator(snapshot); next.updatedAt = nowISO(); pushUndo(prev); return next; }); }
  ...
  const filtered = useMemo(() => { if (!query) return orderedTeeth; const q=query.toString().toLowerCase(); return orderedTeeth.filter(t => t.toString().toLowerCase().includes(q)); }, [orderedTeeth, query]);
    ...
    <div className="w-full p-4 md:p-6 lg:p-8 bg-gradient-to-b from-white to-slate-50 text-slate-900">
      ...
      <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 pr-3 mr-2 border-r">
          ...
          <input placeholder="Find tooth…" value={query} onChange={e=>setQuery(e.target.value)} className="rounded-xl border px-3 py-2 shadow-sm bg-white"/>
        </div>
        <div className="flex items-center gap-2 pr-3 mr-2 border-r">
          <StickyNote className="w-5 h-5"/>
          <input placeholder="Clinician (optional)" value={clinician} onChange={e=>setClinician(e.target.value)} className="rounded-xl border px-3 py-2 shadow-sm bg-white"/>
          <input placeholder="Note (optional)" value={note} onChange={e=>setNote(e.target.value)} className="rounded-xl border px-3 py-2 shadow-sm bg-white"/>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-8 md:grid-cols-16 gap-2">
            {upper.filter(t=>filtered.includes(t)).map(t=>(
              <ToothSVG key={t} toothId={t} entry={state.teeth[t]||EMPTY_TOOTH(t)} selected={selectedTooth===t} onSelect={setSelectedTooth} onSurfaceClick={handleSurfaceApply}/>
            ))}
          </div>
        </div>
        <div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="rounded-2xl shadow-md">
          <CardContent className="p-3 md:p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {CONDITIONS.map(c => (
                <Button key={c.id} onClick={()=>setSelectedCondition(c.id)} variant={selectedCondition===c.id?"default":"outline"} className="rounded-xl flex items-center gap-1 px-2 py-1">
                  <span>{c.icon}</span> <span className="text-xs">{c.label}</span>
                </Button>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="rounded-2xl shadow-md">
          <CardContent className="p-3 md:p-4 flex flex-col gap-2">
            ...
            <Button onClick={async()=>{ const loaded = await loadChart(patientId); if(loaded) setState(loaded); }} className="rounded-xl" variant="outline">Load from Supabase</Button>
